...
class UNPoolingLayerTest : public MultiDeviceTest<TypeParam> {
 ...
 protected:
  ...
  void TestMaxPoolingForwardSquare() {
      ...
      blob_bottom_vec.push_back(blob_bottom_mask_);
    ...
    // Test for 2x 2 square pooling layer
    void TestAvePoolingForwardSquare() {
        vector<Blob<Dtype>*> blob_bottom_vec;
        vector<Blob<Dtype>*> blob_top_vec;
        ...
        //     [1 2 5 2 3]                        
        for (int i = 0; i < 8 * num * channels; i += 8) {
        blob_bottom_->mutable_cpu_data()[i +  0] = (1+2+9+4)/4.0;
        blob_bottom_->mutable_cpu_data()[i +  1] = (2+5+4+1)/4.0;
        blob_bottom_->mutable_cpu_data()[i +  2] = (5+2+1+4)/4.0;
        blob_bottom_->mutable_cpu_data()[i +  3] = (2+3+4+8)/4.0;
        blob_bottom_->mutable_cpu_data()[i +  4] = (9+4+1+2)/4.0;
        blob_bottom_->mutable_cpu_data()[i +  5] = (4+1+2+5)/4.0;
        blob_bottom_->mutable_cpu_data()[i +  6] = (1+4+5+2)/4.0;
        blob_bottom_->mutable_cpu_data()[i +  7] = (4+8+2+3)/4.0;
      }
      blob_bottom_vec.push_back(blob_bottom_);
      blob_top_vec.push_back(blob_top_);
 
      PoolingLayer<Dtype> layer(layer_param);
      layer.SetUp(blob_bottom_vec_, blob_top_vec);
      EXPECT_EQ(blob_top_->num(), num);
      EXPECT_EQ(blob_top_->channels(), channels);
      EXPECT_EQ(blob_top_->height(), 3);
      EXPECT_EQ(blob_top_->width(), 5);
      ...
      //
      for (int i = 0; i < 15 * num * channels; i += 15) {
        EXPECT_NEAR(blob_top_->cpu_data()[i + 0], 4.0, 1e-6);
        EXPECT_NEAR(blob_top_->cpu_data()[i + 1], 3.5, 1e-6);
        EXPECT_NEAR(blob_top_->cpu_data()[i + 2], 3.0, 1e-6);
        EXPECT_NEAR(blob_top_->cpu_data()[i + 3], 3.625, 1e-6);
        EXPECT_NEAR(blob_top_->cpu_data()[i + 4], 4.25, 1e-6);
        EXPECT_NEAR(blob_top_->cpu_data()[i + 5], 4.0, 1e-6);
        EXPECT_NEAR(blob_top_->cpu_data()[i + 6], 3.5, 1e-6);
        EXPECT_NEAR(blob_top_->cpu_data()[i + 7], 3.0, 1e-6);
        EXPECT_NEAR(blob_top_->cpu_data()[i + 8], 3.625, 1e-6);
        EXPECT_NEAR(blob_top_->cpu_data()[i + 9], 4.75, 1e-6);
        EXPECT_NEAR(blob_top_->cpu_data()[i + 10], 4.0, 1e-6);
        EXPECT_NEAR(blob_top_->cpu_data()[i + 11], 3.5, 1e-6);
        EXPECT_NEAR(blob_top_->cpu_data()[i + 12], 3.0, 1e-6);
        EXPECT_NEAR(blob_top_->cpu_data()[i + 13], 3.625, 1e-6);
        EXPECT_NEAR(blob_top_->cpu_data()[i + 14], 4.25, 1e-6);
      }
    }
  };
}
